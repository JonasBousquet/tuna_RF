import MLPplots as plots
import MLPconfig as config
import MLPpreprocessor as pre
from sklearn.model_selection import (GridSearchCV)
from MLPutils import generate_run_directories, console, save_model, save_params, print_regression_metrics
...
if __name__ == '__main__':
    for i in config.variables:
        ...
        main(data_path=config.path_to_file,
             ...
             model=config.MLPRegressor,
             model_param_grid=config.test_params if config.test else config.MLPRegressor,
             test_size=0.2,
             random_state=42,
             cv=5,
             variables=variables,
